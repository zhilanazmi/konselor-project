...
@section('title', 'Edit Kelas - KonselorKita')

@section('content')
<div class="card">
    <div class="card-header">
        <h6 class="text-lg font-semibold mb-0">Edit Kelas: {{ $classroom->name }}</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.classrooms.update', $classroom) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $classroom->name) }}" class="form-control @error('name') !border-danger-600 @enderror">
                    @error('name')
                        <p class="text-danger-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <select id="grade" name="grade" class="form-control @error('grade') !border-danger-600 @enderror">
                        ...
                        <option value="7" {{ old('grade', $classroom->grade) === '7' ? 'selected' : '' }}>Kelas 7</option>
                        <option value="8" {{ old('grade', $classroom->grade) === '8' ? 'selected' : '' }}>Kelas 8</option>
                        <option value="9" {{ old('grade', $classroom->grade) === '9' ? 'selected' : '' }}>Kelas 9</option>
                    </select>
                    @error('grade')
                        <p class="text-danger-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
