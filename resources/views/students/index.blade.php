...
@section('title', 'Data Siswa - KonselorKita')
...
<div class="card">
    <div class="card-header flex flex-wrap items-center justify-between gap-4">
        <h6 class="text-lg font-semibold mb-0">Daftar Siswa</h6>
        <div class="flex flex-wrap items-center gap-3">
            <form action="{{ route('admin.students.index') }}" method="GET" class="flex items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama / NIS..." class="form-control !rounded-lg">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.students.index') }}" class="btn btn-outline-secondary !rounded-lg">Reset</a>
                @endif
            </form>
            <a href="{{ route('admin.students.create') }}" class="btn btn-primary-600 flex items-center gap-2 !rounded-lg">
                ...
                <tbody>
                    @forelse($students as $index => $student)
                        <tr>
                            ...
                            <td>
                                @php
                                    $activeClassroom = $student->classrooms->first(fn ($c) => $c->academicYear?->is_active);
                                @endphp
                                {{ $activeClassroom?->name ?? '-' }}
                            </td>
                            <td class="!text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('admin.students.edit', $student) }}" class="w-8 h-8 bg-primary-50 dark:bg-primary-600/25 text-primary-600 rounded-lg flex items-center justify-center hover:bg-primary-100" title="Edit">
