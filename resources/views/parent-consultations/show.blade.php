...
@php
    $statusColors = ['requested'=>'bg-warning-100 text-warning-600','scheduled'=>'bg-sky-100 text-sky-600','completed'=>'bg-success-100 text-success-600'];
    ...
    <div class="card-body">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    <h5 class="text-xl font-bold mb-0">{{ $consultation->guardian->full_name }}</h5>
                ...
                <p class="text-neutral-500 dark:text-neutral-400 mb-0 text-sm">
                    Siswa: <strong class="text-neutral-700 dark:text-neutral-200">{{ $consultation->student->full_name }}</strong>
                    ({{ $consultation->student->nis }}) &bull; {{ $consultation->academicYear->name }}
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('guru-bk.parent-consultations.pdf', $consultation) }}" target="_blank"
        ...
        <div class="mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-700 flex flex-wrap gap-6 text-sm">
            ...
            <div class="flex items-center gap-2 text-neutral-500 dark:text-neutral-400">
                ...
                <span>Pemohon: <strong class="text-neutral-700 dark:text-neutral-200">{{ $consultation->requested_by == 'guru_bk' ? 'Guru BK' : 'Orang Tua' }}</strong></span>
            </div>
            <div class="flex items-center gap-2 text-neutral-500 dark:text-neutral-400">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    <div class="card lg:col-span-2">
        <div class="card-header"><h6 class="font-semibold mb-0 flex items-center gap-2"><iconify-icon icon="solar:chat-round-line-bold" class="text-primary-600"></iconify-icon>Topik Pembahasan</h6></div>
        <div class="card-body"><p class="whitespace-pre-wrap mb-0">{{ $consultation->topic }}</p></div>
    </div>
    @if($consultation->notes)
    <div class="card lg:col-span-2">
        <div class="card-header"><h6 class="font-semibold mb-0 flex items-center gap-2"><iconify-icon icon="solar:notes-bold" class="text-neutral-500"></iconify-icon>Catatan</h6></div>
        <div class="card-body"><p class="whitespace-pre-wrap mb-0">{{ $consultation->notes }}</p></div>
    </div>
    @endif
    <div class="card">
        <div class="card-header"><h6 class="font-semibold mb-0 flex items-center gap-2"><iconify-icon icon="solar:document-add-bold" class="text-info-600"></iconify-icon>Hasil Konsultasi</h6></div>
        <div class="card-body">
            @if($consultation->result)<p class="whitespace-pre-wrap mb-0">{{ $consultation->result }}</p>
    ...
    <div class="card">
        <div class="card-header"><h6 class="font-semibold mb-0 flex items-center gap-2"><iconify-icon icon="solar:handshake-bold" class="text-success-600"></iconify-icon>Kesepakatan</h6></div>
        <div class="card-body">
            @if($consultation->agreement)<p class="whitespace-pre-wrap mb-0">{{ $consultation->agreement }}</p>
            @else<p class="text-neutral-400 italic mb-0">Belum diisi.</p>@endif
        </div>
    </div>
</div>
