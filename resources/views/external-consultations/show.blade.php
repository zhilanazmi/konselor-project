...
@section('title', 'Detail Konsultasi Pihak Luar - KonselorKita')
...
<div class="card mb-4">
    <div class="card-body">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <h5 class="text-xl font-bold mb-1">{{ $consultation->external_party_name }}</h5>
                <p class="text-neutral-500 dark:text-neutral-400 mb-0 text-sm">
                    <span class="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-600">{{ $consultation->external_party_role }}</span>
                    @if($consultation->student)
                        &bull; Siswa: <strong class="text-neutral-700 dark:text-neutral-200">{{ $consultation->student->full_name }}</strong>
                    @endif
                    &bull; {{ $consultation->academicYear->name }}
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('guru-bk.external-consultations.pdf', $consultation) }}" target="_blank"
                ...
                <a href="{{ route('guru-bk.external-consultations.edit', $consultation) }}" class="btn btn-primary-600 !rounded-lg flex items-center gap-2 !py-1.5 !text-sm">
...
<div class="mt-4">
    <a href="{{ route('guru-bk.external-consultations.index') }}" class="btn btn-outline-secondary flex items-center gap-2 w-fit">
