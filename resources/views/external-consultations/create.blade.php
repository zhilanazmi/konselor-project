...
@section('title', 'Tambah Konsultasi Pihak Luar - KonselorKita')

@section('content')
<div class="card">
    <div class="card-header">
        <h6 class="text-lg font-semibold mb-0">Form Konsultasi Pihak Luar</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('guru-bk.external-consultations.store') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label for="external_party_name" class="form-label">Nama Pihak Luar <span class="text-danger-600">*</span></label>
                    <input type="text" id="external_party_name" name="external_party_name" value="{{ old('external_party_name') }}"
                        class="form-control @error('external_party_name') !border-danger-600 @enderror"
                        placeholder="Contoh: Budi Santoso, Dr. Rina, dll.">
                ...
                <div>
                    ...
                    <input type="text" id="external_party_role" name="external_party_role" value="{{ old('external_party_role') }}"
                        class="form-control @error('external_party_role') !border-danger-600 @enderror"
                        placeholder="Contoh: Orang Tua, Psikolog, Dokter, dll.">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="datetime-local" id="consultation_date" name="consultation_date" value="{{ old('consultation_date') }}"
            ...
            <div class="mb-4">
                ...
                <textarea id="topic" name="topic" rows="3" class="form-control @error('topic') !border-danger-600 @enderror"
                    placeholder="Uraikan topik atau permasalahan yang dikonsultasikan...">{{ old('topic') }}</textarea>
                @error('topic')<p class="text-danger-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="mb-4">
                <label for="notes" class="form-label">Catatan</label>
                <textarea id="notes" name="notes" rows="3" class="form-control" placeholder="Catatan tambahan...">{{ old('notes') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('guru-bk.external-consultations.index') }}" class="btn btn-outline-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
