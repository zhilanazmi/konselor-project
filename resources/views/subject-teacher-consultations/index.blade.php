...
@section('title', 'Konsultasi Guru Mata Pelajaran - KonselorKita')
...
<div class="card">
    <div class="card-header flex flex-wrap items-center justify-between gap-4">
        <h6 class="text-lg font-semibold mb-0">Daftar Konsultasi Guru Mata Pelajaran</h6>
        <a href="{{ route('guru-bk.subject-teacher-consultations.create') }}" class="btn btn-primary-600 flex items-center gap-2 !rounded-lg">
    ...
    <div class="card-body border-b border-neutral-200 dark:border-neutral-700 pb-4">
        <form action="{{ route('guru-bk.subject-teacher-consultations.index') }}" method="GET" class="flex flex-wrap items-end gap-3">
            <div class="flex-1 min-w-[200px]">
                <label class="form-label text-xs">Cari Siswa / Guru / Mapel</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama siswa, guru, atau mata pelajaran..." class="form-control !rounded-lg">
            </div>
            ...
            <div class="min-w-[160px]">
                <label class="form-label text-xs">Guru Mapel</label>
            ...
            <div class="flex gap-2">
                ...
                @if(request()->hasAny(['search', 'academic_year_id', 'teacher_id']))
                    <a href="{{ route('guru-bk.subject-teacher-consultations.index') }}" class="btn btn-outline-secondary !rounded-lg">Reset</a>
                @endif
            </div>
        </form>
    </div>
                ...
                <tbody>
                    @forelse($consultations as $index => $consultation)
                        <tr>
                            ...
                            <td>
                                <span class="px-2 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
                            ...
                            <td class="!text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('guru-bk.subject-teacher-consultations.show', $consultation) }}" class="w-8 h-8 bg-info-50 dark:bg-info-600/25 text-info-600 rounded-lg flex items-center justify-center hover:bg-info-100" title="Detail">
                                        <iconify-icon icon="solar:eye-bold" class="text-lg"></iconify-icon>
                                    </a>
                                    <a href="{{ route('guru-bk.subject-teacher-consultations.edit', $consultation) }}" class="w-8 h-8 bg-primary-50 dark:bg-primary-600/25 text-primary-600 rounded-lg flex items-center justify-center hover:bg-primary-100" title="Edit">
                        ...
                        <tr>
                            <td colspan="7" class="!text-center py-8">
                                <div class="flex flex-col items-center gap-2 text-neutral-400">
                                    ...
                                    <p class="mb-0">Belum ada data konsultasi guru mata pelajaran.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
