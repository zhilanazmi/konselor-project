...
@section('title', 'Tambah Konsultasi Guru Mapel - KonselorKita')

@section('content')
<div class="card">
    <div class="card-header">
        <h6 class="text-lg font-semibold mb-0">Form Tambah Konsultasi Guru Mata Pelajaran</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('guru-bk.subject-teacher-consultations.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" id="subject_name" name="subject_name" value="{{ old('subject_name') }}"
                        ...
                        placeholder="Nama mata pelajaran yang bermasalah">
                    @error('subject_name')
                        <p class="text-danger-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-neutral-400 mt-1">Bisa diisi otomatis saat memilih guru di atas.</p>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="topic" name="topic" rows="4" class="form-control @error('topic') !border-danger-600 @enderror" placeholder="Uraikan permasalahan akademik siswa di mata pelajaran ini...">{{ old('topic') }}</textarea>
                @error('topic')
                    <p class="text-danger-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="recommendation" name="recommendation" rows="3" class="form-control" placeholder="Rekomendasi yang diberikan kepada guru...">{{ old('recommendation') }}</textarea>
            </div>

            <div class="mb-6">
                <label for="follow_up" class="form-label">Tindak Lanjut</label>
                <textarea id="follow_up" name="follow_up" rows="3" class="form-control" placeholder="Rencana tindak lanjut...">{{ old('follow_up') }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('guru-bk.subject-teacher-consultations.index') }}" class="btn btn-outline-secondary">Batal</a>
            </div>
        </form>
    </div>
</div>
...
@push('scripts')
<script>
    // Auto-fill subject_name when selecting a teacher
